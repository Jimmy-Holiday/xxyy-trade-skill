...
These two endpoints share ~90% identical response structure. The key differences are noted below.

| | KOL Buy List | Tag Holder Buy List |
|---|---|---|
| **Endpoint** | `GET ${XXYY_API_BASE_URL:-https://www.xxyy.io}/api/trade/open/api/kol-buy-list?chain={chain}` | `GET ${XXYY_API_BASE_URL:-https://www.xxyy.io}/api/trade/open/api/tag-holder-buy-list?chain={chain}` |
| **Description** | Get KOL (Key Opinion Leader) recent buy list. Shows tokens recently purchased by influential traders. | Get tag holder (Smart Money, Whale, etc.) recent buy list. Shows tokens recently purchased by tagged wallets. |
| **`walletBuyCnt` meaning** | Number of KOL wallets that bought | Number of tagged wallets that bought |
| **`walletBuyItemList[].walletName`** | KOL wallet name | Holder wallet name / tag |
...
- **walletBuyCnt**: Number of wallets that bought (KOL wallets or tagged wallets)
- **lastTradeTime**: Last trade timestamp (milliseconds)
- **holder**: Number of holders
- **walletBuyItemList[].wallet**: Wallet address
- **walletBuyItemList[].walletName**: Wallet name (KOL name or holder tag)
